...
const loadModule = (cmpMeta, hostRef, hmrVersionId) => {
    ...
    return __sc_import_trive_uikit(
...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_trive_uikit('./css-shim-33231b86-6211b682.js');
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_trive_uikit('./dom-ab48ffb1-eff5215c.js');
        }
    ...
    if ((BUILD.lazyLoad || BUILD.style) && (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if (BUILD.style && !styles.has(scopeId) && Cstr.style) {
            ...
            if (!BUILD.hydrateServerSide && BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await __sc_import_trive_uikit('./shadow-css-4889ae62-23996f3f.js').then(m => m.scopeCss(style, scopeId, false));
            }
